...
def check_permission_and_not_submitted(doc):
	...
	# check if submitted
	if doc.docstatus == 1:
		frappe.msgprint(_("{0} {1}: Submitted Record cannot be deleted. Please {2} the selected document before deleting it.").format(_(doc.doctype), doc.name, frappe.bold(_("Cancel"))),
